...
export class AnalyticsService {
  ...
  async getVendorAnalytics(userId: string) {
      ...
      [
        ...
        this.prisma.evaluatorReview.findMany({
          ...
          select: {
            ...
            procurement: { select: { id: true, requestNo: true, title: true } },
          },
          orderBy: { submittedAt: 'desc' },
        }),
      ],
    ...
    const winRate =
      evaluations.length > 0
        ? Math.round((highScoreCount / evaluations.length) * 100)
        : 0;
  ...
  async getVendorPerformance(userId: string) {
    ...
    const scoreTimeline = Object.entries(monthlyScores).map(
      ([month, data]) => ({
        month,
    ...
    const scoreDistribution = [
      ...
      {
        range: '80-89',
        count: reviews.filter((r) => r.score >= 80 && r.score < 90).length,
      },
      {
        range: '70-79',
        count: reviews.filter((r) => r.score >= 70 && r.score < 80).length,
      },
      {
        range: '60-69',
        count: reviews.filter((r) => r.score >= 60 && r.score < 70).length,
      },
